...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until).foldLeft(0f) { case (z, i) =>
      List(input(i) / i, z).max
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def rec(i: Int, end: Int, max: Float): Unit = if (i < end) {
      output(i) = List(input(i) / i, max).max
      rec(i + 1, end, output(i))
    }

    rec(from, until, startingAngle)
  }
